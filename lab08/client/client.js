...
const initializeConnection = ev => {
    ...
    connection.onmessage = e => {
        ...
        if (data["type"] === "login" || data["type"] === "disconnect") {
            ...
            users.forEach(user => user_list += `<li>${user}</li>`);
            user_list += `</ul>`
            qs("#users-list").innerHTML = user_list;
        }
            ...
            if (user === username) {
                var m = `<div class="right"><b>You:&emsp;</b>${message}</div>`
                qs("#chat").innerHTML += m;
            }
            else {
                var m = `<div class="left"><b>${user}:&emsp;</b>${message}</div>`
                qs("#chat").innerHTML += m;
            }
        }
